...
def load_portfolio_from_s3(filename: str = None) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    """
...
def save_portfolio_to_s3(portfolio_df: pd.DataFrame, filename: str = None) -> Tuple[bool, Optional[str]]:
    """
...
def list_portfolios_in_s3() -> List[str]:
    ...
    try:
        ...
        filenames = [f.replace(PORTFOLIO_S3_PREFIX, '') for f in files if f.endswith('.csv')]
        return filenames
    except Exception as e:
        return []
...
def calculate_portfolio_value(portfolio_df: pd.DataFrame, current_prices: Dict[str, float] = None) -> pd.DataFrame:
    """
