...
def detect_language_heuristic(text: str) -> Tuple[str, float]:
    ...
    french_words = ['le', 'la', 'les', 'de', 'du', 'des', 'et', 'un', 'une', 'par', 'pour', 'dans', 'sur']
...
def is_xml_content(content: str) -> bool:
    ...
    return (content_stripped.startswith('<?xml') or 
            ...
            '<XML>' in content_stripped[:500] or
            content_stripped.startswith('<bill') or
            content_stripped.startswith('<publicLaw'))
...
def extract_dates(text: str, language: str = 'en') -> List[Dict[str, str]]:
    ...
    for match in re.finditer(year_pattern, text):
        ...
        if any(word in context.lower() for word in ['effective', 'enact', 'pass', 'date', 'year', '生效', '施行', '発効']):
            dates.append({
                'date': match.group(0),
                'context': context,
                'type': 'year'
...
def extract_financial_impacts(text: str, language: str = 'en') -> List[Dict[str, str]]:
    ...
    currency_patterns = {
        'en': [r'\$\s*([\d,]+(?:\.[\d]+)?)\s*(?:million|billion|trillion|M|B|T)?', 
               r'([\d,]+(?:\.[\d]+)?)\s*(?:million|billion|trillion|M|B|T)?\s*(?:USD|dollars?)',
               r'([\d,]+(?:\.[\d]+)?)\s*(?:million|billion|trillion|M|B|T)?\s*(?:EUR|euros?)'],
        'fr': [r'([\d,]+(?:\.[\d]+)?)\s*(?:million|milliard|millions|milliards|M|Md)?\s*(?:EUR|euros?)',
               r'€\s*([\d,]+(?:\.[\d]+)?)\s*(?:million|milliard|millions|milliards|M|Md)?'],
        'zh': [r'([\d,]+(?:\.[\d]+)?)\s*(?:万元|亿元|万元人民币|亿元人民币|美元|欧元)',
               r'[￥$€]\s*([\d,]+(?:\.[\d]+)?)\s*(?:万元|亿元)?'],
        'ja': [r'([\d,]+(?:\.[\d]+)?)\s*(?:万円|億円|百万円|円)',
               r'[￥$€]\s*([\d,]+(?:\.[\d]+)?)\s*(?:万円|億円)?']
    }
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            ...
            penalty_keywords = {
                'en': ['penalty', 'fine', 'sanction', 'violation', 'non-compliance', 'funding', 'cost', 'tax'],
                'fr': ['amende', 'sanction', 'violation', 'financement', 'coût', 'taxe', 'pénalité'],
...
def extract_affected_sectors(text: str, language: str = 'en') -> List[str]:
    ...
    sector_keywords = {
        'en': {
            'energy': ['energy', 'power', 'electricity', 'renewable', 'solar', 'wind', 'nuclear', 'coal', 'oil', 'gas'],
            'technology': ['technology', 'artificial intelligence', 'ai', 'machine learning', 'data', 'software', 'digital', 'cyber'],
            'finance': ['financial', 'banking', 'investment', 'securities', 'trading', 'market'],
            'healthcare': ['healthcare', 'medical', 'pharmaceutical', 'drug', 'medicine', 'health'],
            'telecommunications': ['telecommunication', 'telecom', 'communication', 'network', 'internet', '5g'],
            'automotive': ['automotive', 'vehicle', 'car', 'truck', 'transportation'],
            'manufacturing': ['manufacturing', 'production', 'factory', 'industrial'],
            'consumer': ['consumer', 'retail', 'e-commerce', 'shopping'],
            'environmental': ['environmental', 'emission', 'carbon', 'climate', 'pollution', 'green'],
            'agriculture': ['agriculture', 'farming', 'food', 'crop']
        },
        'fr': {
            'energy': ['énergie', 'électricité', 'renouvelable', 'solaire', 'éolien', 'nucléaire', 'charbon', 'pétrole', 'gaz'],
            'technology': ['technologie', 'intelligence artificielle', 'ia', 'données', 'numérique', 'cyber'],
            'finance': ['financier', 'banque', 'investissement', 'sécurité', 'trading', 'marché'],
            'consumer': ['consommateur', 'commerce', 'e-commerce', 'achat']
        },
        'zh': {
            'energy': ['能源', '电力', '可再生能源', '太阳能', '风能', '核能', '煤炭', '石油', '天然气'],
        ...
        'ja': {
            'energy': ['エネルギー', '電力', '再生可能エネルギー', '太陽光', '風力', '原子力', '石炭', '石油', 'ガス'],
            'technology': ['技術', '人工知能', 'AI', 'データ', 'ソフトウェア', 'デジタル'],
...
def extract_compliance_requirements(text: str, language: str = 'en') -> List[str]:
    ...
    requirement_keywords = {
        'en': ['shall', 'must', 'required', 'mandatory', 'obligation', 'compliance', 'implement', 'enforce'],
        'fr': ['doit', 'nécessaire', 'obligatoire', 'obligation', 'conformité', 'mettre en œuvre', 'appliquer'],
        'zh': ['应当', '必须', '要求', '义务', '合规', '实施', '执行'],
        'ja': ['しなければならない', '必要', '義務', 'コンプライアンス', '実施', '執行']
    }
...
def extract_geographic_scope(text: str, language: str = 'en') -> List[str]:
    ...
    regions_keywords = {
        'en': ['United States', 'US', 'USA', 'European Union', 'EU', 'China', 'Japan', 'United Kingdom', 'UK', 
               'global', 'international', 'national', 'regional', 'state', 'federal'],
        'fr': ['États-Unis', 'États Unis', 'UE', 'Union européenne', 'Chine', 'Japon', 'Royaume-Uni', 
               'mondial', 'international', 'national', 'régional', 'fédéral'],
        'zh': ['美国', '欧盟', '欧洲', '日本', '英国', '全球', '国际', '国家', '地区', '联邦'],
        'ja': ['米国', 'アメリカ', 'EU', '欧州連合', '中国', '英国', 'グローバル', '国際', '国内', '地域']
    }
...
def extract_sections_from_directive(html_content: str, is_xml: bool = False, 
    ...
    try:
        ...
        provision_keywords = {
            'en': ['preamble', 'summary', 'purpose', 'objective', 'scope', 'article', 'section', 'chapter'],
            'fr': ['préambule', 'résumé', 'objet', 'objectif', 'portée', 'article', 'section', 'chapitre'],
        ...
        if provisions_start > 0:
            sections["key_provisions"] = '\n'.join(lines[provisions_start:provisions_start+50])
        else:
        ...
        keywords = penalty_keywords.get(language, penalty_keywords['en'])
        penalty_lines = [line for line in lines if any(keyword in line.lower() for keyword in keywords)]
        if penalty_lines:
            sections["penalties_sanctions"] = '\n'.join(penalty_lines[:30])
        
        # Extract amendments and references
        reference_keywords = {
            'en': ['amend', 'modify', 'reference', 'cite', 'directive', 'regulation', 'law'],
        ...
        ref_lines = [line for line in lines if any(keyword in line.lower() for keyword in keywords)]
        if ref_lines:
...
def save_extracted_directive_to_s3(directive_name: str, sections: Dict[str, str],
                                   language: str,
                                   portfolio_name: str = "default") -> str:
    """
    ...
    try:
        ...
        # Filter for files matching this directive
        matching_files = [f for f in existing_files if directive_name in f and f.endswith('.json')]
...
def extract_directive_from_file(file_path: str,
                               portfolio_name: str = "default",
                               save_to_s3: bool = True) -> Dict[str, str]:
    """
    ...
    try:
        ...
        if save_to_s3:
            try:
                directive_name = os.path.basename(file_path).replace('.html', '').replace('.xml', '')
...
def extract_directives_from_directory(directory_path: str,
                                      portfolio_name: str = "default",
                                      save_to_s3: bool = True) -> List[Dict[str, str]]:
    """
    ...
    for file_path in all_files:
        try:
            result = extract_directive_from_file(
                file_path, portfolio_name, save_to_s3
            )
