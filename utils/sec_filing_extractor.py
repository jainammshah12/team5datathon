...
def parse_sec_filing(html_content: str, filing_type: str = "10-K") -> tuple:
    ...
    try:
        ...
        print(f"[INFO] Successfully parsed {filing_type} filing with {len(elements) if elements else 0} semantic elements")
...
def extract_sections_from_html(html_content: str, is_xml: bool = False) -> Dict[str, str]:
    """
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if (('item 1' in line_lower or 'item 1.' in line_lower or 'item1' in line_lower) 
                and 'business' in line_lower 
                and 'item 1a' not in line_lower and 'item 1.a' not in line_lower and 'item1a' not in line_lower
                and 'item 1b' not in line_lower and 'item 1.b' not in line_lower and 'item1b' not in line_lower):
            ...
            elif (('item 1a' in line_lower or 'item 1.a' in line_lower or 'item1a' in line_lower) 
            ...
            elif (('item 1c' in line_lower or 'item 1.c' in line_lower or 'item1c' in line_lower) 
            ...
            # Item 2: Properties
            elif (('item 2' in line_lower or 'item 2.' in line_lower or 'item2' in line_lower) 
            ...
            elif (('item 3' in line_lower or 'item 3.' in line_lower or 'item3' in line_lower) 
            ...
            elif (('item 5' in line_lower or 'item 5.' in line_lower or 'item5' in line_lower) 
                  and ('market' in line_lower or 'equity' in line_lower or 'stockholder' in line_lower)):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif (('item 7' in line_lower or 'item 7.' in line_lower or 'item7' in line_lower) 
                  and ('management' in line_lower or 'discussion' in line_lower or 'mda' in line_lower or 'md&a' in line_lower)
                  and 'item 7a' not in line_lower and 'item 7.a' not in line_lower):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
                current_section = "mda"
                section_lines = [line]
            ...
            # Item 7A: Market Risk
            elif (('item 7a' in line_lower or 'item 7.a' in line_lower or 'item7a' in line_lower) 
            ...
            elif (('item 8' in line_lower or 'item 8.' in line_lower or 'item8' in line_lower)
            ...
            elif (('item 9a' in line_lower or 'item 9.a' in line_lower or 'item9a' in line_lower) 
            ...
            elif (('item 11' in line_lower or 'item 11.' in line_lower or 'item11' in line_lower) 
            ...
            elif ('1.01' in line_lower or 'item 1.01' in line_lower) and ('agreement' in line_lower or 'definitive' in line_lower):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('1.05' in line_lower or 'item 1.05' in line_lower) and 'cyber' in line_lower:
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('2.01' in line_lower or 'item 2.01' in line_lower) and ('acquisition' in line_lower or 'disposition' in line_lower):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('2.02' in line_lower or 'item 2.02' in line_lower) and ('results' in line_lower or 'financial' in line_lower):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('2.06' in line_lower or 'item 2.06' in line_lower) and 'impairment' in line_lower:
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('4.01' in line_lower or 'item 4.01' in line_lower) and 'accountant' in line_lower:
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('5.01' in line_lower or 'item 5.01' in line_lower) and 'control' in line_lower:
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            elif ('5.02' in line_lower or 'item 5.02' in line_lower) and ('officer' in line_lower or 'director' in line_lower):
                if current_section and section_lines:
                    sections[current_section] = '\n'.join(section_lines)
            ...
            # Stop at next Item to avoid overlapping sections (more lenient threshold)
            elif current_section and line_lower.startswith('item ') and len(section_lines) > 20:
                ...
                is_new_tracked_item = any([
                    ...
                    'item 5' in line_lower and ('market' in line_lower or 'equity' in line_lower),
                    'item 7' in line_lower and ('management' in line_lower or 'discussion' in line_lower),
                    ...
                    'item 11' in line_lower and ('executive' in line_lower or 'compensation' in line_lower)
                ])
        ...
        for section_name, content in sections.items():
            if section_name != "full_text" and content:
                ...
                print(f"[INFO] Extracted {section_name}: {char_count} chars, {word_count} words, {line_count} lines")
...
def save_extracted_sections_to_s3(ticker: str, sections: Dict[str, str], 
                                   portfolio_name: str,
    ...
    s3_prefix = f"/data/extracted_filings/{portfolio_name}/{ticker}/"
    ...
    try:
        ...
        matching_files = [f for f in existing_files if filing_type_normalized in f and f.endswith('.json')]
        
        if matching_files:
            print(f"[INFO] Cleaning up {len(matching_files)} old extraction file(s) from S3...")
    ...
    output_data = {
        ...
        "statistics": {
            ...
            "sections_found": [k for k in sections.keys() if sections.get(k) and k != "full_text" and not k.startswith('_')],
            "total_sections_count": len([k for k in sections.keys() if sections.get(k) and k != "full_text" and not k.startswith('_')])
        },
    ...
    s3_key = f"/data/extracted_filings/{portfolio_name}/{ticker}/{filename}"
    ...
    try:
        ...
        print(f"[INFO] Sections extracted: {', '.join(output_data['statistics']['sections_found'])}")
        return s3_key
    except Exception as e:
...
def extract_filing_from_html(ticker: str, html_content: str, 
                              portfolio_name: str,
                              filing_type: str = "10-K",
                              save_to_s3: bool = True,
                              is_xml: bool = False) -> Dict[str, str]:
    """
    ...
    """
    try:
        print(f"[INFO] Processing {filing_type} filing for {ticker} (Portfolio: {portfolio_name})...")
        ...
        try:
            ...
            if elements:
                ...
                if rendered:
                    ...
                    print(f"[INFO] Generated rendered content from {len(elements)} semantic elements")
        except Exception as e:
        ...
        if save_to_s3:
            try:
                s3_key = save_extracted_sections_to_s3(ticker, sections, portfolio_name, filing_type)
        ...
        sections['_summary'] = {
            ...
            'sections_found': [k for k in sections.keys() if sections.get(k) and not k.startswith('_')]
        }
...
def extract_key_filing_sections(ticker: str, html_content: str, 
    ...
    Returns:
        Dictionary with extracted sections and metadata
    """
    return extract_filing_from_html(ticker, html_content, portfolio_name, filing_type, save_to_s3, is_xml)
