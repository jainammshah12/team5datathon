...
def read_csv_from_s3(key: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ConnectionError(f"Failed to read CSV '{key}' from S3 bucket '{bucket_name}': {e}")
...
def read_file_from_s3(key: str) -> str:
    ...
    try:
        ...
        raise ConnectionError(f"Failed to read '{key}' from S3 bucket '{bucket_name}': {e}")
...
def delete_file_from_s3(key: str) -> None:
    ...
    try:
        ...
        raise ConnectionError(f"Failed to delete '{key}' from S3 bucket '{bucket_name}': {e}")
