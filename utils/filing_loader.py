...
def load_extracted_filing_for_ticker(ticker: str) -> Optional[Dict]:
    """
    Load the most recent extracted filing JSON for a ticker from extracted_filings/{ticker}/.
    ...
    """
    try:
        # List files in extracted_filings/{ticker}/
        prefix = f"extracted_filings/{ticker}/"
        ...
        json_files = [
            f
            for f in files
            if f.endswith(".json") and ("10_k" in f.lower() or "10k" in f.lower())
        ]
...
def get_relevant_sections_for_analysis(
    filing_data: Dict, max_chars_per_section: int = 2000
) -> Dict[str, str]:
    """
